...
function solve_constant_depth_decision_rules_depth_one(subi_a,subj_a,valij_a,betas,cost,probabilities,ds,T,ps,qs)
    time1 = @elapsed begin
        ...
        #Variables
        @variable(CDDR1, v[t = 1:T, i = 1:ps[t], s = 1:t, k = 1:ds[s]])
        @constraint(CDDR1, v[1,:,:,:] .<= 0)
        @variable(CDDR1, u[t = 1:T, i = 1:qs[t], s = 1:t, k = 1:ds[s]])
        ...
        for t=1:T, s=1:t, i=1:ps[t], eta=1:ds[s]
            ...
            @constraint(CDDR1, v[t,i,s,eta] - transpose(val)*var == -betas[t][s][eta,i])
            nconst += 1
        end
        ...
        for t=2:T, i=1:ps[t], s=2:t, k=1:ds[s]
            @constraint(CDDR1, z[t,i,s] - v[t,i,s,k] >= 0)
            nconst += 1
        end
